...
$sql1 = "SELECT id,email FROM users WHERE user_type='patient'";
    ...
    <div id="content">
        ...
        <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
            ...
            <div class="badge badge-secondary text-center col-md-4">
                <!-- Topbar Search -->
                <h3 class="h3 mb-0 text-white-800"> Schedule </h3>
            </div>
        ...
        <div class="container">
            ...
            appointment.id,
            users.id AS uid,
            users.fullname AS patient_fullname,
        ...
        FROM 
            appointment 
        INNER JOIN 
            users ON users.id = appointment.user_id 
        INNER JOIN 
            users AS dentist ON dentist.id = appointment.dentist_id 
        WHERE 
            appointment.send = 0 
            AND appointment.patient_id = '$userid' 
        ORDER BY 
            appointment.Dtime DESC
        ";
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            if ($row['paid']===0) {
                                                //echo "Unpaid";
                                                echo '<a href="pay.php?pay=' . $row['id'] . '&uid=' . $row['uid'] . '&d_id=' . $row['dentist_id'] . '&page=pappointments" class="badge badge-success p-2">Pay</a>';
                                            }else {
                                                echo "Paid";
                                             }
                                             ?>
                                                <!-- <a href="pay.php?pay=<?= $row['id']; ?>&uid=<?= $row['uid']; ?>&d_id=<?= $row['dentist_id']; ?>&page=pappointments" class="badge badge-success p-2">Pay</a> -->
                                            </td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
